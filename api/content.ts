...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    return res.json(await writeSite({ projects: req.body.projects, videoUrl: req.body.videoUrl || null }, current.revision));
  } catch (e) {
    console.error('Content operation failed', e instanceof Error ? e.name : 'unknown');
